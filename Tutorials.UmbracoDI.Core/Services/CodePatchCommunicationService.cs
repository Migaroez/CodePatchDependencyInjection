...
{
    public class CodePatchCommunicationService : ICommunicationService
    {
        private readonly ITextDazzler _textDazzler;

        public CodePatchCommunicationService(ITextDazzler textDazzler)
        {
            _textDazzler = textDazzler;
        }
        ...
        public async Task SendCommentToHost(string name, string email, string comment)
        {
            ...
            message.Body = _textDazzler.Dazzle(comment);
